...
import com.daliedu.dao.UserDao;
			...
			public void onClick(View v) {
				...
				if (checkInput()) {
					if(checkNetWork())
					{
						...
						new Thread() {
								public void run() {
									String url = Constant.DOMAIN_URL+"mobile/login?username="+name+"&password="+password+"&deviceId="+deviceId;
									String result = null;
									appContext.setLoginState(AppContext.LOGINING);
									try {
										result = HttpConnectUtil.httpGetRequest(
												(AppContext) LoginActivity.this.getApplication(), url);
										if (!"".equals(result)) {
											try {
												// 解析字符串
	...
	private static class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case -2:
				...
				break;
			case -3:
				Toast.makeText(login, "用户名或密码错误,或者非常用登录设备", Toast.LENGTH_SHORT).show();
				break;
			}
		}
